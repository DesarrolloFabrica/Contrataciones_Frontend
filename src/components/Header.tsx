...
const Header: React.FC<HeaderProps> = ({ mode, onChangeMode }) => {
  ...
  const handleLogout = useCallback(() => {
    const logoutFn = auth?.logout ?? auth?.signOut ?? auth?.logoutUser;

    // 1) si existe método oficial, úsalo
    if (typeof logoutFn === "function") {
      logoutFn();
      return;
    }

    // 2) fallback (por si aún no existe logout en el context)
    try {
          ...
          <div className="flex items-center gap-6">
            ...
            <div className="p-1 rounded-xl bg-[#0F0F0F] border border-white/10 shadow-inner flex items-center relative">
              ...
              >
                <LayoutGrid
                  size={14}
                  className={mode === "analyze" ? "text-emerald-500" : "text-neutral-600"}
                />
                Analizar
              </button>
              ...
              >
                <History
                  size={14}
                  className={mode === "history" ? "text-cyan-500" : "text-neutral-600"}
                />
                Historial
              </button>
            </div>
            ...
            <button
              ...
              className="group inline-flex items-center gap-2 px-4 py-2 rounded-xl border border-white/10 bg-white/[0.03]
                         text-xs font-extrabold uppercase tracking-widest text-white/70
                         hover:bg-white/[0.06] hover:text-white transition"
              title="Cerrar sesión"
            >
              <LogOut className="w-4 h-4 text-white/50 group-hover:text-white/80 transition" />
